...
Java_com_alien_gif_GifTranscoder_transcodeInternal(JNIEnv *env, jclass clazz,
                                                   jstring file_path,
                                                   jstring out_file_path) {
    const char *pathIn = env->GetStringUTFChars(file_path, JNI_FALSE);
    const char *pathOut = env->GetStringUTFChars(out_file_path, JNI_FALSE);

    GifTranscoder transcoder;
    int gifCode = transcoder.transcode(pathIn, pathOut);

    env->ReleaseStringUTFChars(file_path, pathIn);
    env->ReleaseStringUTFChars(out_file_path, pathOut);
